...
```{r ans13}
meanEcon <- sum(grade*fromEcom)/sum(fromEcom)
meanEcon
...
```{r ans23}
hour(twTime)
nightowl <- hour(twTime) == c(1:6)
nightowl

hour_twTime <- hour(twTime)
nightOwl1 <- hour_twTime >=1 & hour_twTime<=6
nightOwl1
```
...
nightOwlVoteShare1 <- sum(nightOwl1)/length(nightOwl1)
nightOwlVoteShare1
```
...
```{r ans34}
subject <- (ischosenStudent & schoolYear==100 & credit >0)

gpa100ChosenStudent <- sum(subject*grade*credit)/sum(subject*credit)
gpa100ChosenStudent
```
